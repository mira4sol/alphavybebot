...
import { connection, isMintAddress } from '@/utils/solana.lib'
...
export const messageCommand = async (ctx: TelegrafContext) => {
  try {
    ...
    if (ctx.session?.waitingForInput) {
      ...
      const input = parseFloat(ctx.text || '')
      if (isNaN(input)) {
      ...
      switch (ctx.session.waitingForInput) {
        case 'auto_buy_amount_sol':
          ...
          updateData.auto_buy_amount_sol = input
          break
        case 'left_buy_amount_sol':
          if (input <= 0) throw new Error('Amount must be greater than 0')
          updateData.buy_amount_sol = input
          break
        case 'right_buy_amount_sol':
          if (input <= 0) throw new Error('Amount must be greater than 0')
          updateData.max_buy_amount_sol = input
          break
        case 'sell_partial_percentage':
          if (input <= 0 || input > 100)
            throw new Error('Percentage must be between 0 and 100')
          updateData.sell_partial_percentage = input
          break
        case 'sell_full_percentage':
          if (input <= 0 || input > 100)
            throw new Error('Percentage must be between 0 and 100')
          updateData.sell_full_percentage = input
          break
        case 'custom_buy_amount': {
          ...
          const buyAmountLamports = input * 1e9 // Convert SOL to lamports
